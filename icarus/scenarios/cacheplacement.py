...
__all__ = [
    ...
    "closeness_centrality_cache_placement",
...
def betweenness_centrality_cache_placement(topology, cache_budget, **kwargs):
    ...
    for v in icr_candidates:
        ...
        #print(iround(cache_budget * betw[v] / total_betw))
             	    

@register_cache_placement("Eigenvector")
def eigenvector_centrality_cache_placement(topology, cache_budget, **kwargs):
    try:
   	 eigen = dict(nx.eigenvector_centrality(topology))
    except nx.PowerIterationFailedConvergence as e:
    	eigen = dict(nx.eigenvector_centrality_numpy(topology))
    #print(eigen)
    icr_candidates = set(topology.graph["icr_candidates"])
    total_eigen = sum(v for k, v in eigen.items() if k in icr_candidates)
    for v in icr_candidates:
    	topology.node[v]["stack"][1]["cache_size"] = iround(cache_budget * eigen[v] / total_eigen)
    	#print(topology.node[v]["stack"][1]["cache_size"])
    	
@register_cache_placement("Eigenvector2")
def eigenvector_centrality_cache_placement(topology, cache_budget, **kwargs):
    eigen = dict(nx.eigenvector_centrality(topology))
    #print(len(topology))
    #print(eigen)
...
@register_cache_placement("Eigenvector3")
def eigenvector_centrality_cache_placement(topology, cache_budget, **kwargs):
    #try:
   	 #eigen = dict(nx.eigenvector_centrality(topology))
    #except nx.PowerIterationFailedConvergence as e:
    A = nx.to_numpy_matrix(topology)
    ...
    for i in range (len(topology)):
    	SUM = SUM + (eigvecs[i,0] ** 2)
    eigen = (eigvecs[:,0])/SUM**0.5
    print(eigen)
    u = {int(k) : eigen[k] for k in range (len(eigen))}
    print(u)
    icr_candidates = set(topology.graph["icr_candidates"])
    total_eigen = sum(v for k, v in eigen.items() if k in icr_candidates)
    print (total_eigen)
...
@register_cache_placement("PageRank")
def eigenvector_centrality_cache_placement(topology, cache_budget, **kwargs):
    eigen = dict(nx.pagerank(topology))
    #print(len(topology))
    #print(eigen)
    icr_candidates = set(topology.graph["icr_candidates"])
    total_eigen = sum(v for k, v in eigen.items() if k in icr_candidates)
...
@register_cache_placement("Closeness")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.closeness_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)

    for v in icr_candidates:
        topology.node[v]["stack"][1]["cache_size"] = iround(cache_budget * clo[v] / total_clo)
        
@register_cache_placement("Load")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.load_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Subgraph")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.subgraph_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Harmonic")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.harmonic_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Dispersion")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.dispersion(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Reaching")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.global_reaching_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Percolation")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.percolation_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Second_Order")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.second_order_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Trophic")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.trophic_levels(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("VoteRank")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.voterank(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Laplacian")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.laplacian_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
...
@register_cache_placement("Information")
def closeness_centrality_cache_placement(topology, cache_budget, **kwargs):

    clo = dict(nx.information_centrality(topology))
    icr_candidates = set(topology.graph["icr_candidates"])
    total_clo = sum(v for k, v in clo.items() if k in icr_candidates)
